...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .nav-item.active {
      background: rgba(0, 136, 194, 0.08);
      color: #0088c2;
      border-left-color: #0088c2;
      font-weight: 500;
    }
  ...
  <script>
    ...
    async function processCurrentOrder() {
      ...
      try {
        ...
        const btn = document.getElementById('processBtn');
        if (btn) { btn.disabled = false; btn.textContent = 'Process This Order'; }
      }
    }
    ...
    async function processOrderDirect(orderId) {
      ...
      if (data.processed > 0) {
        ...
        toast('Error: ' + (data.error || 'Processing failed'), 'error');
      }
    }
      ...
      try {
        ...
        document.getElementById('mappingsList').innerHTML = mappings.length ? mappings.map(m => \`
          <div class="mapping-item">
            <span style="flex:1">\${m.edi_customer_name}</span>
            <span class="mapping-arrow">→</span>
            <span style="flex:1">\${m.zoho_account_name}</span>
            <button class="btn btn-danger" style="padding:0.25rem 0.75rem;font-size:0.75rem" onclick="deleteMapping(\${m.id})">Delete</button>
          </div>
    ...
    async function deleteMapping(id) {
      try { await fetch('/customer-mappings/' + id, { method: 'DELETE' }); loadMappings(); } catch(e) {}
    }
        ...
        document.getElementById('draftsList').innerHTML = drafts.length ? drafts.map(d => \`
          <div class="mapping-item">
            <div style="flex:1">
              ...
              <div style="font-size:0.75rem;color:#86868b">Replaced by PO# \${d.edi_po_number} on \${new Date(d.replaced_at).toLocaleDateString()}</div>
            </div>
            <button class="btn btn-secondary" style="padding:0.25rem 0.75rem;font-size:0.75rem">Details</button>
          </div>
